...
/**
 * Candidate session log filenames, in priority order. dsh 0.1.5+ migrated to
 * the v3 Session format (`session.v3.jsonl[.zstd]`); the legacy
 * `session.jsonl[.zstd]` is kept on disk but no longer written. A moved
 * session must have its header `cwd` rewritten in EVERY present generation,
 * otherwise dsh's persistence layer rejects the path identity mismatch as
 * "corrupt session log".
 */
...
/** A session log filename is one of the canonical generations (with/without zstd). */
function isSessionLogFilename(file) {
  return SESSION_LOG_CANDIDATES.includes(file)
}

/**
 * List the canonical session log files actually present in a session dir,
 * ordered by priority (current v3 generation first, legacy last).
 */
function presentSessionLogs(dir) {
  try {
    const present = new Set(readdirSync(dir))
    return SESSION_LOG_CANDIDATES.filter((file) => present.has(file))
  } catch {
    return []
  }
}
...
async function moveSessionToWorkspace(ctx, sessionId, targetWorkspaceId, options = {}) {
  ...
  if (currentPath === null || !existsSync(currentPath)) {
    if (existsSync(root)) {
      for (const projectDirName of readdirSync(root)) {
        ...
        // Prefer the current v3 generation, then fall back to legacy.
        const present = presentSessionLogs(candidateDir)
        if (present.length > 0) {
  ...
  // 6. Rewrite the header cwd in EVERY present session log generation (v3
  //    current + legacy). For zstd logs this is a FRAME-PRESERVING surgery
  //    (decode frame 1, rewrite header, re-encode frame 1, keep frames 2..N
  //    byte-identical) — dsh requires the first frame to contain exactly the
  //    header line. For plain logs it's a plaintext splice. Rewriting every
  //    generation keeps the moved session consistent no matter which format
  //    the running dsh reads (dsh 0.1.5 reads `session.v3.jsonl.zstd`).
  const logFiles = presentSessionLogs(oldSessionDir)
