...
async function moveSessionToWorkspace(ctx, sessionId, targetWorkspaceId, options = {}) {
  ...
  // 8b. Reconcile the in-memory world with the new disk location (rc.8
  //     semantics — identical in rc.6). dsh keeps the live session's frozen
  //     header (`cwd` = old workspace) and the persistence coordinator caches
  //     the old `meta.cwd`; both are used by the NEXT append, which would
  //     then fail (old dir is gone) or be rejected as an id collision. Sync:
  //     - live session: swap its header for one stamped with the new cwd
  //       (the header property is a plain writable field, not frozen);
  //     - coordinator: drop the cached state for this id so the next append
  //       re-adopts from disk (adopt() rescans the session tree);
  ...
  // 9. Move the session's ownership account: detach old / attach new.
