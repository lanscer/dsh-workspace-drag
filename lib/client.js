/**
 * dsh-workspace-drag — client half (browser).
 *
 * @module dsh-workspace-drag/lib/client
 * @description 无感拖拽整理：从侧边栏把一个对话（会话行）拖到另一个工作区分组
 *   上即完成跨工作区迁移。没有浮动弹窗、没有中间面板——拖到哪个工作区（组头
 *   或该组内任意会话行）就直接归到那个工作区。
 * @license MIT
 ...
 *
 * DEPENDENCIES (provided by the DSH web host):
...
window.__ModuleLoader__.load({
	...
	var zh = {
		...
		"settings.waiting": "对话仍在写入，稍候自动完成…",
	...
	var en = {
		...
		"settings.waiting": "Conversation is still writing; finishing automatically…",
	...
	// Move
	// ---------------------------------------------------------------------
	var MOVE_WAIT_MS = 15000;

	function moveOnce(ctx, sessionId, workspace) {
		return fetch(MOVE_API, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
				sessionId: sessionId,
				targetWorkspaceId: workspace.workspaceId,
				// Let the host wait for the agent to finish / the log to
				// quiesce instead of failing with "please retry later".
				waitMs: MOVE_WAIT_MS,
			}),
		}).then(function (res) { return res.json(); });
	}
	...
	async function performMove(ctx, sessionId, workspace) {
		...
		try {
			var data = await moveOnce(ctx, sessionId, workspace);
			// The host already waited waitMs; a 'writing' answer means the
			// session kept writing the whole time — one short retry, then fail.
			if (data && data.ok !== true && data.code === "writing") {
				transientBanner("⏳ " + ctx.locale.bind(NS)("settings.waiting"), 4000);
				await new Promise(function (r) { setTimeout(r, 3000); });
				data = await moveOnce(ctx, sessionId, workspace);
			}
